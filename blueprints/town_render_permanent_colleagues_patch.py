...
(()=>{
  ...
  function finishStep(p){const s=p.current;if(!s)return;const id=stepId(s);if(id)p.done.add(id);if(s.type==='say')p.speech='';if(s.type==='leave')p.visible=false;p.current=null;}
  function tick(p,dt){
    if(!p.visible)return;
    if(!p.current&&p.queue.length)startStep(p,p.queue.shift());
    const s=p.current;if(!s)return;s.elapsed+=dt;
  ...
  function merge(roster,world){
    ...
    extras.forEach((row,index)=>{
      ...
      const agent=agentById.get(id)||{};const generic=genericById.get(id)||null;
      const explicitlyOn=String(agent.dutyState||'')==='on';
      const explicitlyOff=String(agent.dutyState||'')==='off'||agent.manualOffDuty===true;
      ...
      if(!p){p={id,name:String(row.name||id),x:135+col*115,y:220+line*42,bodyColor:'#536f86',accentColor:'#d4a74a',visible,queue:[],current:null,done:new Set(),speech:''};people.set(id,p);}
      p.name=String(row.name||id);p.visible=visible;
      const raw=generic||agent;
      if(!p.current&&!p.queue.length){if(Number.isFinite(Number(raw&&raw.x)))p.x=Number(raw.x);if(Number.isFinite(Number(raw&&raw.y)))p.y=Number(raw.y);}
